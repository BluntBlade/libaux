...
UTF8_VERIFY_AGAIN:
    sts = next[sts][tbl[ones.bytes[0] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[1] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[2] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[3] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[4] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[5] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[6] >> 3]]; ok_bytes += sts != VSS_ERROR;
    sts = next[sts][tbl[ones.bytes[7] >> 3]]; ok_bytes += sts != VSS_ERROR;
